"""Tests for the ``*_to_view`` adapters in ``aaiclick.data.view_models``.

The adapters are exercised end-to-end by
``aaiclick/internal_api/test_objects.py``; ``scope_of`` and
``name_from_table`` have their own tests in
``aaiclick/data/test_scope.py``.
"""
