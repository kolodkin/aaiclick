"""Tests for the ``aai_id`` ergonomic helper on ``create_object_from_value``.

When ``aai_id=True``, ``create_object_from_value`` adds an ``aai_id``
...
async def test_aai_id_true_adds_aai_id_column(ctx):
...
async def test_aai_id_recovers_insertion_order(ctx):
...
async def test_aai_id_dict_of_arrays(ctx):
...
async def test_aai_id_collision_raises(ctx):
