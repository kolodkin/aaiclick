...
All three required; missing any → task returns `AirtablePublishResult(status="skipped", reason="...")` without raising. Mirrors the `HF_TOKEN` gating pattern already used by `publish_to_huggingface`.
