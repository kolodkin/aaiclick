...
- The `@job` function returns `TaskResult(tasks=[report])` with only the terminal task — the framework auto-discovers all upstream tasks via the dependency graph; `report.py` is only responsible for the printout
