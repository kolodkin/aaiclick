"""Test helper: read CH ``task_logs`` from a spawned child process.

For mp-module tests (``orch_ctx_no_ch``) the parent holds no chdb session, so
reading back what a worker/flush child wrote requires another child that opens
its own ``orch_context``.
"""
