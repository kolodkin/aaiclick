...
async def test_test_job_simple(orch_ctx, monkeypatch):
    """Test test_job() executes a simple task synchronously.

    Note: test_job() uses asyncio.run() internally, which is tested
    via test_job_async() in the async context to avoid nested event loops.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        monkeypatch.setenv("AAICLICK_LOG_DIR", tmpdir)
        ...
        # Test execution via the async helper (same code path as test_job())
        await test_job_async(job)
