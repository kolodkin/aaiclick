...
async def test_local_runtime_rebuilds_when_the_marker_outlives_the_schema(monkeypatch):
    ...
    set up. ``stale_local_db`` cannot see this — it compares columns of tables
    that exist — so the helper checks for missing tables and re-runs setup
    instead of starting workers that fail on the first query."""
    monkeypatch.setattr(lr, "is_local", lambda: True)
    monkeypatch.setattr(lr, "is_setup_done", lambda: True)
    monkeypatch.setattr(lr, "missing_local_tables", lambda: ["jobs", "tasks"])
    monkeypatch.setattr(lr, "stale_local_db", lambda: [])
    called: list[bool] = []
