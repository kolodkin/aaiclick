...
async def _stream(client: httpx.AsyncClient) -> asyncio.Task[httpx.Response]:
...
async def test_event_frames_drops_its_subscriber_on_cancel():
    """A client disconnect cancels the stream mid-wait. White-box on purpose:
    a leaked subscriber is invisible from outside, and a browser leaks one per
    navigation."""
    bus = EventBus()
...
async def test_events_stream_frames_a_change(app_client, bus):
    request = await _stream(app_client)
    bus.publish()
