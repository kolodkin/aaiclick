...
async def test_login_then_access_protected(orch_ctx, app_client, enabled):
    ...
    login = await app_client.post(
        f"{API_PREFIX}/auth/login", json={"username": "alice", "password": "pw"}
    )
    assert login.status_code == 200
    access = login.json()["access_token"]

    me = await app_client.get(
        f"{API_PREFIX}/auth/me", headers={"Authorization": f"Bearer {access}"}
    )
...
async def test_login_bad_password_401(orch_ctx, app_client, enabled):
    ...
    res = await app_client.post(
        f"{API_PREFIX}/auth/login", json={"username": "alice", "password": "x"}
    )
...
async def test_refresh_flow(orch_ctx, app_client, enabled):
    ...
    res = await app_client.post(
        f"{API_PREFIX}/auth/refresh", json={"refresh_token": login["refresh_token"]}
    )
