...
async def test_oplog_disabled_by_default():
    """No oplog=True produces no collector and no operation_log entries."""
...
async def test_oplog_collector_created_when_enabled():
    """data_context(oplog=True) installs an OplogCollector in the ContextVar."""
...
async def test_copy_recorded():
    ...
    async with data_context(oplog=True):
        collector = get_oplog_collector()
        a = await create_object_from_value([7, 8, 9])
        result = await a.copy()
...
async def test_flush_writes_to_operation_log():
    ...
    async with data_context(oplog=True):
        obj = await create_object_from_value([100, 200, 300])
...
async def test_flush_writes_to_table_registry():
    ...
    async with data_context(oplog=True):
        obj = await create_object_from_value([1])
        table_name = obj.table
