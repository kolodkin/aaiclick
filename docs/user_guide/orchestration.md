...
| `AAICLICK_WAIT_TIMEOUT`              | `3600`                                | Seconds `job wait` / `run-job --progress` watch a job before giving up. Bounds the wait, not the job — a timeout stops watching and exits 1 while the job keeps running |
