...
Snowflake ID column present in every Object table. Provides temporal ordering across distributed operations — ClickHouse doesn't guarantee insertion order in SELECT.
...
Timestamp-encoded unique identifier generated by ClickHouse via `generateSnowflakeID()`. Encodes creation time at millisecond precision, providing natural temporal ordering across distributed operations without explicit `ORDER BY`.
