...
**Goal:** Prepare the target data shape — extend `ColumnView`/`SchemaView` with `fieldtype` fields and add a `schema_json` column to the SQL `table_registry` — without changing any runtime behaviour yet.

**Depends on:** nothing. Execute first.

**Unlocks:** Phase 2 (which starts reading `schema_json` back out).
...
| `aaiclick/orchestration/lifecycle/db_lifecycle.py`                                                | Modify — add `schema_json: str \| None` column on `TableRegistry`.        |
| `aaiclick/orchestration/migrations/versions/<new>_add_schema_json_to_table_registry.py`           | Create — Alembic migration.                                               |
...
### Task 1.2: Add `schema_json` column to `TableRegistry` SQLModel
...
Expected: FAIL with `TypeError: 'schema_json' is an invalid keyword argument for TableRegistry`.
...
git commit -m "feature: add nullable schema_json column to TableRegistry"
```

---

### Task 1.3: Alembic migration for `schema_json`
...
```bash
alembic -c aaiclick/orchestration/migrations/alembic.ini revision -m "add schema_json to table_registry"
```
...
```bash
git add aaiclick/orchestration/migrations/versions/<generated>_add_schema_json_to_table_registry.py
git commit -m "feature: alembic migration adding schema_json to table_registry"
```
...
- `TableRegistry.schema_json` is a nullable `Text` column at the ORM level **and** in the migration history.
- No runtime code writes or reads `schema_json` yet — that's Phases 2 and 3.
