...
**Goal:** Drop the hidden `aai_id` column and ClickHouse column `COMMENT`s, move schema metadata into a `schema_doc` column on the SQL `table_registry`, and tighten the operator contract so cross-table binary ops and `.data()` no longer rely on an implicit ordering key.
...
|---|----------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|
| 1 | [Registry schema & Pydantic model](./2026-04-24-remove-aai-id-and-comments_phase_1.md)             | `fieldtype` on `ColumnView`/`SchemaView`; Alembic migration adds `schema_doc` to `table_registry`.   |
| 2 | [Schema reads from registry](./2026-04-24-remove-aai-id-and-comments_phase_2.md)                   | `_get_table_schema` reads `table_registry.schema_doc`; `Schema` ↔ `SchemaView` conversion helpers.   |
...
3. **Registry row invariant holds** — every `create_object` / copy / ingest path that creates a ClickHouse table also writes a `table_registry` row with `schema_doc`.
