...
## Operation Provenance Integration ✅ IMPLEMENTED

**Implementation**: `aaiclick/orchestration/execution.py` — see `execute_task()`

`OplogCollector(task_id=task.id, job_id=task.job_id)` is created and passed as `data_context(oplog=collector)` for every task execution. Collector auto-flushes on clean exit. Integration tests in `aaiclick/orchestration/test_provenance_lifecycle.py`.

Remaining: AI agents as `@task` wrappers — lazy import, participates in normal DAG dependencies.
...
On each cleanup cycle, completed/failed/cancelled job tables from `table_registry` are replaced with 10-row samples (same name, same schema). Sampled tables are removed from `table_context_refs` so lifecycle cleanup won't drop them — they persist indefinitely for AI lineage queries. Integration tests in `aaiclick/orchestration/test_provenance_lifecycle.py`.
