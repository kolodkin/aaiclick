...
A standalone JVM worker (`java -jar aaiclick-worker.jar [--max-tasks N]`, built from `java/aaiclick-worker`) claims and runs **host-subprocess shell tasks only**, so module and container tasks stay on Python workers. Both workers execute the **same claim query** — the shared contract in `aaiclick/orchestration/execution/sql/claim_next_task.sql`, embedded into the jar at build time — with per-worker capabilities passed as bound values (`entry_types`, `allow_image_tasks`), never query edits. Distributed-only by construction (refuses chdb/SQLite URLs); no Object/View support. On failure it just sets `PENDING_CLEANUP` — retries and cleanup remain the Python `BackgroundWorker`'s job — and its only ClickHouse touchpoints are `generateSnowflakeID()` and `task_logs` inserts over HTTP.
