...
async def test_append_scalar_fails():
    """Test that appending to a scalar object raises ValueError."""
    a = await create_object_from_value(42)
    b = await create_object_from_value([1, 2, 3])
...
async def test_append_chained():
    """Test chaining multiple append operations."""
    a = await create_object_from_value([1, 2])
    b = await create_object_from_value([3, 4])
    c = await create_object_from_value([5, 6])

    temp = await a.append(b)
    result = await temp.append(c)
    data = await result.data()
